...
기존 ``readers/`` 패키지(COM 기반, 의미 단위 ``DocItem`` 까지 생성)와 역할이 다르다.
여기서는 openpyxl/python-docx 로 파일을 열어(크로스플랫폼·Office 불필요) 순수
데이터클래스(:mod:`contentcompare.raw.models`)로 변환만 한다.
"""
