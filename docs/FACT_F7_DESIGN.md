...
**근거 검증 게이트가 실제로 막는 것**(과대평가하지 말 것). 이 게이트는 인용문의 토큰이
해당 fact 의 원문에 실재하는지만 본다(커버리지 ≥ `EVIDENCE_MIN_COVERAGE`, 인용 토큰 수
≥ `EVIDENCE_MIN_TOKENS`). 따라서:

- 막는 것: **원문에 없는 문구를 지어내는 것**, 그리고 **풍부한 원문에서 한 조각만 떼어내는
  부분 인용**(`공칭용량, 1150, mAh` 에서 `1150` 만).
- 막지 못하는 것: **관련 없는 두 fact 를 잇는 것 자체.** LLM 이 양쪽 원문에서 실재하는
  문구를 골라 인용하면 두 fact 가 무관해도 게이트는 통과한다.

즉 이 게이트는 "주장의 출처가 실재하는가"를 보증할 뿐 "주장이 옳은가"를 보증하지 않는다.
잘못된 연결을 실제로 막는 것은 (a) LLM 의 판단 자체(§5 프롬프트 규약), (b) 승격된
`differs_by`, (c) 하류의 `needs_review` — LLM 이 새로 만든 연결 위에서는 값 판정도 LLM 이
한 번 더 보게 하는 것(§4 F5 Matcher 변경) — 이다. 한글 토큰이 bigram 으로 부풀려지는 것도
토큰 수 하한을 약화시키므로, 하한은 "퇴화한 인용을 막는 바닥"으로만 이해해야 한다.

**토큰 하한의 예외 — 원문 전체 인용**(2026-08-05 수정). 하한이 막는 것은 *짧은 인용*이
아니라 *부분 인용*이다. 원문 자체가 짧으면(엑셀 단일값 행의 `evidence_text = "1150.0"`)
하한을 채울 방법이 없어 **달성 불가능한 조건**이 된다. 그래서 인용 토큰 집합이 그 fact 의
`evidence_text` 토큰 집합과 같으면 하한을 면제한다(`concept_assembler._quote_long_enough`).

이것을 드러낸 것은 영어 대상 문서 실측이다. 언어가 다르면 `norm_name` 완전일치가 0건이라
**모든 연결이 LLM 경로로 몰리고**, 그제서야 이 게이트가 전면에 걸린다 — 한국어 문서에서는
같은 행들이 `BY_CODE` 로 이어져 검증을 건너뛰었기 때문에 가려져 있었다. 수정 전에는 LLM 이
정확히 인용한 `same_as` 10건이 전부 강등되어 실제 불일치(공칭전압 3.89 vs 3.85V)를 놓쳤다.
회귀는 `tests/test_concept_regression_en.py`.

**토큰 하한의 예외 ② — 항목명 전체 인용**(2026-08-06 수정). LLM 이 값이 아니라 **이름**을
인용하는 경우다. 개념 판정의 주장이 "이 둘은 같은 항목이다"이므로 이름을 근거로 드는 것은
오히려 주장에 맞다 — 실측에서 LLM 의 사유가 "공칭용량의 영문 표기는 Nominal capacity
입니다"였다. 그런데 `tokenize` 가 한글 3글자 이상을 bigram 으로 부풀리는 탓에 `공칭용량`
(4 토큰)은 통과하고 `Nominal capacity`(2 토큰)는 막혔다 — **같은 성격의 인용이 언어에
따라 갈렸다.** 커버리지는 1.00 으로 원문에 완벽히 실재했고, 대상 원문
(`Nominal capacity, 1150, mAh`)이 이름보다 길어 예외 ①로도 구제되지 않았다.
그래서 인용이 그 fact 의 `entity_name` 토큰을 **통째로 포함**하면(부분집합 검사) 면제한다.

부분집합인 것이 중요하다 — `최대충전전류` 에서 한정어를 뺀 `충전전류` 는 이름을 통째로
담지 못하므로 면제되지 않는다. 이것이 §12 의 잘못된 병합
(`표준충전전류 ↔ Maximum charging current`)을 만든 실패 유형이다.

> ⚠️ **하한 자체가 언어 편향적이라는 사실은 남는다.** 위 `충전전류` 인용은 한국어에서
> 4 토큰이라 하한을 그냥 통과하므로 면제 ②의 부분집합 검사가 걸리지 않는다. 즉 한정어
> 탈락 인용은 **영어에서만 막히고 한국어에서는 통과한다.** 근본 해결은 하한을 없애고
> "인용이 항목명을 포함하는가"를 **필수 조건**으로 바꾸는 것인데, 그러면 값만 인용하는
> 정상 경로(예외 ①)가 깨지므로 값 인용을 요구하지 않도록 프롬프트를 먼저 바꿔야 한다.
> 관측된 오판이 나오면 그때 착수한다(§3.2 의 같은 원칙).
...
```yaml
# 같은 개념의 여러 표기(번역어·약칭). 그룹 안은 서로 same_as 이고,
# 아래 관계 entry 의 이름은 이 그룹 전체로 확장된다.
aliases:
  - ["표준환경온도", "Standard ambient temperature"]
  - ["평가환경습도", "Evaluation ambient humidity"]
...
**`aliases` 가 왜 별도 섹션인가**(2026-08-05 추가). 번역어를 `differs_by` 의 `names` 에
그냥 나열하면 안 된다 — 로더가 목록 안 **모든 쌍**에 같은 관계를 부여하므로
`["1개월저장온도", "표준환경온도", "Standard ambient temperature"]` 는 뒤 둘(동의어)까지
"다르다"로 선언한다. `aliases` 로 묶으면 관계 entry 가 **그룹 간 쌍만** 만들고 그룹 내부는
건너뛰므로, `1개월저장온도 ↔ Standard ambient temperature` 차단이 자동으로 따라온다.

**한계**: 온톨로지는 사람이 승격한 뒤에만 효과가 있다. **처음 보는 외국어 문서**는 여전히
구제하지 못한다 — 재발 방지 장치이지 첫 실행 대책이 아니다.
...
> ⚠️ **실측: 이 실패 모드가 실제로 발생했다**(2026-08-05, `gemma4:12b`). LLM 이
> `3개월저장온도 ↔ 3개월 저장 조건`(상한 70 vs 60)을 `axis="상한값"`,
> 사유 "상한 범위 값이 서로 다릅니다"로 **차단**해, 찾아야 할 불일치가 `missing` 이 됐다.
> `aliases` 유무와 무관하게 재현됐고(A/B 0건 차이), 다른 실행에서는 `axis` 를 `"기간"` 으로
> 붙이고 사유에만 값 근거를 남겨 **`axis` 어휘 검사로는 걸러지지 않았다**(47건 중 1건).
> 코드가 `axis` 를 해석하지 않는다는 §2.1 원칙과도 충돌하므로, 대응은 **프롬프트 강화와
> 계측**이다(후속 과제). 상세는 [`FACT_F3_5_LIVE_REPORT.md`](FACT_F3_5_LIVE_REPORT.md) §11.6.
...
근거 검증은 **LLM 이 제안한 `same_as`**(`decided_by == BY_LLM`)에만 적용한다. 사람이
온톨로지로 승격했거나(`BY_ONTOLOGY`) 코드가 정규화 이름 완전일치로 확정한(`BY_CODE`)
연결은 LLM 의 주장이 아니라 지어낼 수 없으므로 검증 대상이 아니다 — 온톨로지 확정은
애초에 인용문을 채우지도 않는다(사람은 항목명 쌍만 적는다). `differs_by` 는 차단
방향이라 근거가 없어도 손해가 없다(§2.3 비대칭 권한).
...
- **F3 추출 누락**(Word `w_b004` 미추출 등)은 그대로 `missing` 이다. 영어 문서 실측에서도
  규격 버전 문장이 fact 로 추출되지 않아 `고객 표준 버전`이 `missing` 이었다.
...
- **단일값 fact 의 과잉 병합.** 원문이 `"1150"` 뿐인 fact 는 근거 인용으로 자신을 구별할
  수단이 없어 **아무 `1150` 과도 이어질 수 있다.** 실측(2026-08-05 영어 문서)에서 실제로
  `공칭용량 ↔ Rated capacity` 와 `정격용량 ↔ Nominal capacity` 가 교차 병합되어 네 fact 가
  한 개념이 됐다(값이 모두 1150 이라 판정은 바뀌지 않았다). 이름 완전일치가 앵커 역할을
  하는 같은 언어 문서에서는 일어나지 않는다. 현재 방어선은 (a) `needs_review=True` →
  값 판정도 LLM 이 재확인, (b) Comparator 가 후보 여럿을 LLM 에 함께 주고 고르게 함,
  (c) 사람의 온톨로지 승격이다. **실제 오판이 관찰되면 그때 대응한다**(관찰 전에 만들지
  않는다 — §3.2 의 같은 원칙).
