...
일으키는 스위치다. **롤백 스위치는 아니다** — 내려도 프롬프트가 원복되지 않는 이유는
§11 참고.
...
**맥락 블록만 근거로 든 fact 는 차단된다 — 그러나 그것이 전부는 아니다.**
`_facts_from_blocks()` 는 이미 `batch_ids` 로 `source_ids` 를 검증한다
(`fact_extractor.py:165,173`). 맥락 블록 id 를 `batch_ids` 에 넣지 않으므로, **오직**
맥락 블록만 근거로 하는 fact 는 자동으로 드롭된다. 겹쳐 넣고 사후에 중복을 지우는
방식은 쓰지 않는다 — 중복 판정 기준이 또 하나의 추측이 되기 때문이다(**이 코드
판단은 유지한다** — 아래 빈틈은 별도 검사가 아니라 계측으로 대응한다, §8).

⚠️ 그런데 프롬프트가 유도하는 행동은 정반대다 — 바로 위 규칙이 "레이블이 생략된
줄·블록은 앞의 레이블에 딸린 것일 수 있다"고 가르친다. LLM 이 맥락 블록과 신규
블록을 **함께** 인용하면(예: `source_ids: ["w_b005", "w_b006"]`, `w_b005` 가 맥락)
`valid_ids` 는 비지 않고 `["w_b006"]` 만 남아 **이 fact 는 드롭되지 않고 살아남는다.**
그런데 `w_b005` 는 직전 배치에서 이미 자기 몫의 fact 를 냈을 수 있다 →
내용이 겹치는 fact 2건. 즉 **차단되는 것은 "맥락만 근거인 fact"뿐**이고, "맥락+신규를
함께 인용한 fact"(바로 위 규칙이 유도한 행동)는 차단 대상이 아니다. 20블록마다 최대
3블록이 이 경계 창이고, 재추출 후 골든 대조에서 경계 블록 근처 동명 fact 증가 여부를
확인한다(`FACT_F3_5_LIVE_REPORT.md` §13.3).
...
**`FACT_VERSION` 을 `fact-v2` 로 되돌려도 프롬프트는 원복되지 않는다.** 실제 롤백은
**이 브랜치를 `git revert` 하는 것**이다.

근거: 프롬프트를 바꾸는 요소는 `FACT_SYSTEM`/`FACT_VERSION` 만이 아니다. 표 행 렌더
(`prompts.py::_render_table`)와 배치 맥락(`fact_extractor.py::_with_context`)은
**버전 상수와 무관하게 무조건 동작**한다. 상수만 내리면 두 경우 모두 조용히 틀린다:

- 줄 구조가 **있는** Word 문서 → `lines_by_block` 이 지문에 섞여 payload 가 달라져
  재계산은 되지만, 렌더·맥락 코드는 그대로라 **새 프롬프트로 계산된다.** 재계산됐다는
  사실이 옛 프롬프트로 돌아갔다는 뜻이 아니다.
- 줄 구조가 **없는** Word 문서(실측: `samples/요약.docx`·`samples/사업보고서.docx` 는
  여러 줄 문단 0개·`cell_lines` 0개 — `FACT_F3_5_LIVE_REPORT.md` §13.2) → payload 가
  v2 시절과 **바이트 동일** → **v2 산출물이 캐시 히트로 서빙된다.** 코드는 새
  프롬프트를 쓰는데 결과는 옛것이고, 한 실행 안에서 문서마다 세대가 섞인다.

`FACT_VERSION` 은 **재추출을 강제하는 스위치일 뿐 롤백 스위치가 아니다** — 값을
올리면 캐시를 무효화해 재계산을 강제할 수는 있지만, 내린다고 프롬프트가 예전으로
돌아가지는 않는다. `raw/` 의 새 필드(`indent`·`cell_lines`)는 additive 이고
`text`/`rows` 를 건드리지 않으므로 `git revert` 없이 남아 있어도 무해하다.
`lines_by_block` 인자는 기본 `None` 이라 넘기지 않으면 기존 경로 그대로다(단, 이
경로 무변경은 렌더·맥락 코드의 무조건 동작과는 별개다).
