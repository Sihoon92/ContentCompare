...
def test_sdk_is_imported_only_after_the_trust_store_is_injected(fake_truststore,
                                                                monkeypatch):
    """순서가 load-bearing 이다 — 이 테스트가 실측 버그를 고정한다.

    truststore 는 ``ssl.SSLContext`` 를 갈아끼우는 방식이라 **다른 모듈이 SSL
    컨텍스트를 만들기 전에** 불러야 한다. langfuse 를 먼저 import 하면 그 과정에서
    httpx/OTLP 익스포터가 컨텍스트를 만들어 주입이 늦어진다. 실제로 이 순서가
    뒤집혀 있어서 단독 스크립트는 통과하는데 ``--check`` 만 ConnectError 로 죽었다.
    """
    import builtins
    import types

    monkeypatch.setitem(sys.modules, "langfuse",
                        types.SimpleNamespace(Langfuse="LF"))
    seen: list[list[str]] = []
    real_import = builtins.__import__

    def spy(name, *args, **kwargs):
        if name == "langfuse":
            # langfuse 를 import 하는 시점에 주입이 이미 끝나 있어야 한다.
            seen.append(list(fake_truststore.calls))
        return real_import(name, *args, **kwargs)

    monkeypatch.setattr(builtins, "__import__", spy)

    from contentcompare.llm.tracing import import_langfuse

    cfg = LangfuseConfig(host=HOST, public_key=PUB, secret_key=SEC)
    assert import_langfuse(cfg) == "LF"
    assert seen == [["injected"]], "langfuse 를 truststore 주입보다 먼저 import 했다"


def test_import_langfuse_propagates_missing_sdk(monkeypatch):
    """미설치는 호출부가 '_SDK 미설치_' 안내로 바꿔야 하므로 그대로 올려보낸다."""
...
def test_trust_source_names_what_is_actually_trusted(fake_truststore, real_cert):
    """``--check`` 가 신뢰 출처를 밝혀야 '설정했는데 왜 안 되지'가 반복되지 않는다.

    실측에서 잃은 시간의 대부분이 "truststore 가 먹히긴 한 건가"를 몰라서였다.
    """
    from contentcompare.llm.tracing import trust_source

    def cfg(**kw):
        return LangfuseConfig(host=HOST, public_key=PUB, secret_key=SEC, **kw)

    assert "truststore" in trust_source(cfg())
    assert "corp-ca.pem" in trust_source(cfg(ssl_cert=real_cert))
    assert "검증 꺼짐" in trust_source(cfg(verify_ssl=False))


def test_trust_source_admits_when_only_certifi_is_available(monkeypatch):
    """truststore 미설치를 숨기면 사내망에서 원인을 못 찾는다."""
    monkeypatch.setitem(sys.modules, "truststore", None)

    from contentcompare.llm.tracing import trust_source

    assert "certifi" in trust_source(
        LangfuseConfig(host=HOST, public_key=PUB, secret_key=SEC))
