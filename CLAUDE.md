...
- **요청 한도 대응이 두 층인 이유(`llm/ratelimit.py`)**: 위 HTTP 레벨 처리는 `requests` 를 직접 쓰는 `internal`/`ollama` 만 덮는다. `langchain` 백엔드는 SDK 가 자체 재시도를 하는데 백오프가 몇 초 수준이라 **분당 한도 회복에 못 미친다**. 그래서 `factory.build_clients` 가 `_needs_rate_limit_wrapper()` 로 판단해 `RateLimitedChat`/`RateLimitedEmbedder` 로 감싼다 — `TracedChat` 과 같은 전략이고 **추적보다 바깥**이라 대기 시간이 `duration_ms` 에 섞이지 않는다. 두 층이 겹쳐 5회×60초가 **두 겹**으로 쌓이는 것(최악 10분)을 막으려고, HTTP 레벨에서 이미 처리하는 백엔드는 `handles_rate_limit = True` 를 선언하고 래퍼가 **사후 재시도만** 건너뛴다(사전 스로틀은 적용). `RateLimiter` 는 고정 창이 아니라 **슬라이딩 창**이며(경계에서 두 배가 몰리는 것 방지) 창이 열릴 만큼만 잔다. chat 과 embed 는 같은 키를 쓰므로 서버가 합산해 세고 → **limiter 를 공유**한다(로컬 임베딩 `fastembed`/`onnx` 는 한도를 안 먹으므로 제외). 한도 예외 감지는 `openai` 를 import 하지 않고 상태코드·클래스명·메시지 마커로 **덕 타이핑**하며, 사내 게이트웨이가 429 를 안 줄 수 있어 `rate_limit_status_codes`/`rate_limit_markers` 로 넓힐 수 있다 — 첫 감지 시 예외 타입·상태코드·본문을 `log_print` 로 남기는 것이 그 조정의 근거다(60초 정지를 행으로 오해하는 것도 막는다).
- ⚠️ **래핑 조건은 래퍼의 기능 목록과 1:1 이어야 한다**(`factory._needs_rate_limit_wrapper`). 조건이 인라인 `if` 였을 때 **같은 결함이 두 번** 났다 — `timeout_wait` 누락(db5bd15), 그 수정이 바로 옆의 `rate_limit_wait` 을 못 봐서 또 누락. 둘 다 증상이 같다: **설정에는 있는데 호출 경로에는 없다.** 실측 429(`Model rate limit exceeded (60/60)`)에서 60초 대기가 전혀 안 걸렸고 원인은 감지 실패가 아니라 래퍼 부재였다. 그래서 조건을 이름 붙은 함수로 꺼내 래퍼에 기능을 더하는 사람이 같이 고쳐야 한다는 사실이 보이게 했다. 세 갈래는 ①사전 스로틀(`max_calls_per_minute`) ②429 대기(`rate_limit_wait`×`rate_limit_max_retries`) ③타임아웃 대기(`timeout_wait`×`timeout_max_retries`)이고, **대기 시간이 있어도 재시도 예산이 0 이면 붙이지 않는다**(아무 효과 없이 "대기 60s × 0" 같은 경고만 낸다). `handles_rate_limit` 은 ②에만 건다 — 그 백엔드는 `http.py` 가 이미 429 를 기다려 주므로 ②만 보고 감싸면 **아무 일도 안 하는 껍데기**가 된다. ③에는 안 건다(그 플래그는 429 만 뜻한다).
- ⚠️ **기본 설정에서 `langchain` 은 이제 `RateLimitedChat` 으로 감싸진다**(`rate_limit_wait` 기본 60). `build_clients` 가 돌려주는 객체의 타입을 단언하는 테스트는 `rate_limit_wait: 0` 으로 꺼서 격리할 것 — `logging.timeline` 을 끄는 것과 같은 이유다.
...
- ⚠️ **화면 중복은 레벨로 막지 않는다**(`logging_setup.NO_CONSOLE`). `log_print` 는 `print` 로 이미 화면에 내보내므로 그 레코드가 콘솔 핸들러로 또 나가면 안 되는데, 예전 주의사항("`level` 을 INFO 로 두면 WARNING 인 콘솔이 걸러 준다")은 **전제가 틀렸다** — 콘솔 핸들러는 WARNING 이었던 적이 없다(아래). 지금은 `ConsoleFilter` 가 `NO_CONSOLE` 표시가 붙은 레코드를 콘솔에서만 걸러내고 파일에는 그대로 남긴다. 새로 `log_print` 류를 만들면 레벨이 아니라 이 표시를 쓸 것 — `--verbose`(콘솔 INFO)에서도 안 무너지는 유일한 방법이다.
- **구조화 출력(`llm.structured_output`, 기본 `auto`)**: 프롬프트로 "JSON 만 출력하세요"라고 **부탁**하는 대신 서버가 문법적으로 그것만 만들 수 있게 **강제**한다. 실측 배경 — F2 배치 7/13 에서 JSON 파싱이 2회 연속 실패해 문서가 통째로 죽었는데, 출력은 잘리지도 비지도 않은 **완전한 응답의 문법 오류**였고 재시도는 `temperature=0` 에 같은 입력을 다시 보내므로 원리적으로 무력했다("2번 실패"는 모델이 헛디딘 게 아니라 `retries=1` 예산이 정확히 2였던 것이다).
  - **pydantic 은 응답이 아니라 요청에 붙는다.** 와이어 모델 → `model_json_schema()` → strict 보정(`llm/structured.py`) → `response_format` 으로 전송 → 서버가 생성 단계에서 강제. 응답 경로(`parse_json_object` → `from_llm` → dataclass)는 **무수정**이라 기존 도메인 모델이 그대로 산다. `model_validate_json` 을 파서로 쓰는 것과 혼동하지 말 것 — 그건 `json.loads` 만큼 엄격해서 문법 오류를 못 고친다.
  - **보장 범위**: 문법·필드 존재·타입·enum 은 어길 수 없게 된다. **내용의 정확성은 아니다** — 지어낸 값·`evidence_text` 할루시네이션은 그대로 가능하고, 그건 지금처럼 코드가 막는다(`_quote_in_evidence` 인용 검증, `source_ids` 교집합, 좌표 덮어쓰기). **그 방어선을 이 기능으로 걷어내지 말 것.**
  - **스키마 전달은 세 번째 덕 타이핑 규약**이다(`handles_rate_limit`·`last_usage` 에 이어). 백엔드가 `supports_structured_output` 을 선언하고, `LlmRunner._call_kwargs` 가 **매 호출** `getattr` 로 물어 참일 때만 `schema=` 를 넘긴다. ⚠️ 이유는 **테스트의 가짜 chat 37개가 `**kwargs` 를 안 받기 때문**이다 — 무조건 넘기면 전부 `TypeError` 다. 플래그를 `__init__` 에 캐시하지 말 것(강등이 반영 안 돼 남은 수백 회가 같은 400 에 부딪힌다).
  - ⚠️ **`__getattr__` 위임이 여기서는 제약이 아니라 해법이다.** `RateLimitedChat(TracedChat(Backend))` 에서 플래그가 두 위임을 타고 백엔드까지 내려간다. 위임이 위험한 것은 *메서드*(호출이 래퍼를 건너뜀)일 때이고 *플래그*일 때는 정확히 필요한 동작이다. 그래서 새 **메서드**가 아니라 기존 `complete()` 의 선택 인자로 갔다.
  - 인자 이름이 `response_format` 이 아니라 **`schema`(JSON Schema dict)** 인 것은 백엔드 중립성 때문이다 — OpenAI 는 `response_format`, Ollama 는 `format` 에 스키마를 날것으로 받는다. 봉투 모양을 아는 것은 백엔드의 일이다.
  - **`with_structured_output()` 을 안 쓴 결정적 근거는 반환 타입이 아니라 `usage.py` 규약이다.** 그쪽은 `AIMessage` 가 아닌 pydantic 객체를 돌려줘 `from_response` 가 토큰을 못 읽는데, 여기서 **미상은 "서버가 안 줬다"는 뜻**이라 우리가 잃어버린 것이 서버 탓으로 기록되고 `tok_per_sec`(배치 크기 판단 근거)이 영영 사라진다. 추가로 `TracedChat` 의 `output` 이 모델 원문이 아니게 되고, 파싱이 SDK 안으로 들어가 `parse_failures` 계측(F4b 입력)을 우회한다. 그래서 `chat.bind(response_format=...)` 로 **문자열 계약을 유지**한다.
  - **서버가 400 을 주면** 스키마 없이 한 번 더 부르고 이 실행 동안 스스로 강등한다(화면·타임라인·`run_stats.structured_calls` 세 곳에 흔적). 판정(`looks_like_schema_rejection`)은 **의도적으로 좁다** — 상태코드와 마커를 둘 다 본다. 넓히면 인증 실패·프롬프트 초과에도 재시도해 모든 실패의 비용이 두 배가 된다(`is_rate_limit` 이 셋 중 하나로 넓게 잡는 것과 반대 이유다).
  - **와이어 모델(`fact/wire_models.py`)은 저장 모델과 일부러 다르다.** 코드가 덮어쓰는 필드(`Fact.fact_id`·`source`·`search_text`, `Record.source.sheet`·`cell_range`)가 없어야 한다 — strict 는 **모든 속성을 required 로 만들어서**, 남겨 두면 "무시됨"이 아니라 "LLM 이 반드시 지어내야 함"이 된다. "중복이니 하나로" 합치는 리팩터링을 `test_code_owned_fields_never_appear_on_the_wire` 가 막는다.
  - **`attributes`/`metadata` 는 와이어만 배열**(`[{name,value,unit}]`)이고 저장은 계속 map 이다. strict 가 자유 키 object 를 금지하기 때문이며, 변환은 `record_models.parse_attributes`/`parse_metadata` **한 곳**에서만 일어나 캐시 히트와 새 응답이 같은 결과로 수렴한다.
  - ⚠️ **pydantic 은 최상위 import 금지.** 이 저장소는 파이썬 환경이 둘이고(운영 anaconda 에는 langchain 을 통해 딸려 오지만 개발용 `.venv` 에는 코어 의존성뿐) 테스트가 양쪽에서 돈다. `fact/schemas.py` 경계층이 import 를 함수 안으로 밀어 넣고 없으면 `None`(=오늘과 같은 동작)을 돌려주므로 **폴백 경로가 매일 테스트된다**. 단 스키마 결함(`ValueError`)은 삼키지 않는다 — 환경 문제와 코드 문제는 조치가 다르다.
  - 지원 여부는 `python scripts/probe_structured_output.py` 로 확인한다(대조군·`json_object`·`json_schema` 3발, 상태코드와 에러 본문을 그대로 보여 준다).
...
**화면은 `setup_console()` 이 소유한다 — `logging.basicConfig` 를 쓰지 말 것.** `basicConfig` 는 **루트 로거**의 레벨만 정하고 자신이 만드는 `StreamHandler` 는 `NOTSET`(=필터 안 함)으로 두는데, `setup_logging()` 은 파일에 DEBUG 를 담으려고 바로 그 루트를 DEBUG 로 **낮춘다**. 그래서 콘솔의 유일한 문턱이 사라져 **화면이 통째로 열렸다** — 실측 증상이 셋이었고 원인은 이 하나였다: ①`log_print` 의 모든 줄이 두 번(`print` + 콘솔 핸들러) ②프롬프트·HTTP 페이로드(DEBUG)가 터미널에 유출 ③`--quiet` 가 화면을 못 끔(파일 전용 `logger.info` 를 콘솔이 대신 찍음). `setup_console()` 은 핸들러에 **자기 레벨**을 주므로 두 함수의 호출 순서가 더는 화면을 좌우하지 않는다. 이 결함이 초기 커밋부터 살아남은 이유는 `test_logging_setup.py` 가 **stdout 만** 보고 콘솔 핸들러를 아예 안 붙였기 때문이다 — 중복은 stderr 로 나갔다. 그래서 `setup_console(stream=...)` 주입구를 두고 화면 출력을 **세는** 테스트를 붙였다.
...
| `tracing.TracedChat` | LLM 요청/응답, 소요, **토큰 사용량**, 실패, `timeout` 80% 초과 시 **느림 경고** |
...
**토큰 사용량(`llm/usage.py`)** 은 세 백엔드가 서버에서 **받아놓고 버리던** 숫자다 — 같은 값을 `prompt_eval_count`(ollama) · `usage.prompt_tokens`(internal) · `usage_metadata.input_tokens`(langchain) 세 이름으로 부르므로 정규화가 한 곳에 필요했다. **반환값이 아니라 백엔드의 `last_usage` 속성으로 건네는 이유**는 `LLMClient.complete` 의 서명(`-> str`)을 바꾸면 코드 무수정 원칙인 `comparison/`·`readers/` 까지 파급되기 때문이다 — `handles_rate_limit` 과 같은 덕 타이핑 규약이고, `TracedChat` 이 호출 직후 읽어 타임라인에 얹는다. ⚠️ **글자 수에서 토큰을 추정하지 말 것** — 서버가 안 주면 키를 통째로 빼서 미상임을 드러낸다. `output_tokens=0` 이 줄에 찍히면 '0개를 썼다'로 읽히는데 실제 뜻은 '서버가 안 알려줬다'로 정반대다. 매 호출 **시작 시 리셋**하는 것도 같은 이유다(실패한 호출에 직전 성공의 토큰이 묻어나면 기록이 거짓말을 한다). `tok_per_sec` 은 **출력** 토큰만 센다 — 입력은 한 번에 읽히고(prefill) 출력은 한 토큰씩 생성되므로, 배치를 얼마나 줄여야 하는지는 생성 쪽 속도가 답한다.
