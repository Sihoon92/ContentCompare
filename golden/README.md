...
| 파일 | 기준 문서 | 대상 문서 | 항목 수 |
|---|---|---|---|
| `자표준_골든셋.jsonl` | `samples/자표준문서.xlsx` (3~22행) | `samples/자표준_규격서.docx`, `samples/자표준_발표.pptx` | 27 (match 17 / mismatch 3 / missing 6 / unknown 1) |
| `spec_en_골든셋.jsonl` | `samples/자표준문서.xlsx` (3~102행 전체) | `samples/spec_en.docx` | 100 (match 50 / mismatch 13 / missing 27 / unknown 10) |
...
기준까지 한 출처로 묶어야 세 쪽이 어긋날 수 없다. 단 **3~22행은 쓰지 않는다**: 그 행의 값과
...
기준 문서(`자표준문서.xlsx`)는 **3~22행과 23~102행의 성격이 다르다**. 둘을 섞어 읽지 말 것.

> ⚠️ 이 경계는 `make_en_sample.REF_FIRST_NEW_ROW` 와 **같이 움직인다**. 기준 엑셀에서 행을
> 지우거나 넣으면 그 상수부터 고칠 것 — 어긋나면 `write_golden()` 이 순번을 못 찾아 죽는다.
> (실제로 f0ad3fe 가 충전온도범위 4행을 지우면서 27 → 23 으로 내려왔다.)

| 행 | 성격 | 라벨에 미치는 영향 |
|---|---|---|
| 3~22 | 손으로 만든 원래 픽스처. **단위 열(K)이 전부 비어 있고** 단일값이 `F(하한치)`·`G(중심치)` 에 섞여 있다 | 실문서의 지저분함이 그대로 남아 있어 `unknown` 이 잘 나온다 |
...
앞의 `unknown` 은 **기준**이 모호한 경우(3~22행), 뒤의 `unknown` 은 **대상**이 모호한 경우다.
